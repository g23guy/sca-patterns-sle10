...
# Title:       PostgreSQL Security Advisory SUSE-SA:2008:005
# Description: PostgreSQL was updated to fix remote code execution security issues, Severity 5 of 10.
...
@PATTERN_RESULTS = (
	...
	"META_LINK_Security=http://www.novell.com/linux/security/advisories/2008_05_postgresql.htm"
);
...
SDP::Core::processOptions();
	...
	if  ( SDP::SUSE::compareKernel(SLE9SP5) <= 0 ) {
		...
		@PKGS_TO_CHECK       = qw(postgresql postgresql-contrib postgresql-devel postgresql-docs postgresql-libs postgresql-libs-32bit postgresql-libs-64bit postgresql-pl postgresql-server);
		$FIXED_VERSION       = '7.4.19-0.1';
