...
# Title:       GnuTLS Security Advisory SUSE-SA:2008:046
# Description: gnutls was updated to fix remote code execution security issues, Severity 8 of 10.
...
@PATTERN_RESULTS = (
	...
	PROPERTY_NAME_COMPONENT."=Gnutls",
	PROPERTY_NAME_PATTERN_ID."=$PATTERN_ID",
	PROPERTY_NAME_PRIMARY_LINK."=META_LINK_Security",
	PROPERTY_NAME_OVERALL."=$GSTATUS",
	PROPERTY_NAME_OVERALL_INFO."=None",
	"META_LINK_Security=http://www.novell.com/linux/security/advisories/2008_46_gnutls.html"
);
...
SDP::Core::processOptions();
	...
	if  ( SDP::SUSE::compareKernel(SLE9SP5) <= 0 ) {
		...
		@PKGS_TO_CHECK       = qw(gnutls gnutls-devel);
		$FIXED_VERSION       = '1.0.8-26.15';
		...
		@PKGS_TO_CHECK       = qw(gnutls gnutls-devel gnutls-32bit gnutls-devel-32bit);
		$FIXED_VERSION       = '1.2.10-13.8.5';
		SDP::Core::printDebug('ARGS', "$CHECKING, $ADVISORY, $TYPE, $#PKGS_TO_CHECK, $FIXED_VERSION");
		SDP::SUSE::securitySeverityPackageCheck($CHECKING, $ADVISORY, $TYPE, \@PKGS_TO_CHECK, $FIXED_VERSION);
	} elsif ( SDP::SUSE::compareKernel(SLE10SP2) <= 0 ) {
		SDP::Core::printDebug('DISTRIBUTION', 'SLE10_SP2');
		@PKGS_TO_CHECK       = qw(gnutls gnutls-devel gnutls-32bit gnutls-devel-32bit);
		$FIXED_VERSION       = '1.2.10-13.11';
		SDP::Core::printDebug('ARGS', "$CHECKING, $ADVISORY, $TYPE, $#PKGS_TO_CHECK, $FIXED_VERSION");
		SDP::SUSE::securitySeverityPackageCheck($CHECKING, $ADVISORY, $TYPE, \@PKGS_TO_CHECK, $FIXED_VERSION);
	} else {
