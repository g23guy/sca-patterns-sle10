...
# Title:       Bind Security Advisory SUSE-SA:2009:005
# Description: bind updated to fix information disclosure problem, Severity 7 of 10.
...
@PATTERN_RESULTS = (
	...
	"META_LINK_Security=http://www.novell.com/linux/security/advisories/2009_05_bind.html"
);
...
SDP::Core::processOptions();
	my $CHECKING            = 'Bind';
	my $ADVISORY            = '7';
	my $TYPE                = 'Information disclosure';
	my @PKGS_TO_CHECK       = ();
	my $FIXED_VERSION       = '';

	if  ( SDP::SUSE::compareKernel(SLE9SP5) < 0 ) {
		SDP::Core::printDebug('DISTRIBUTION', 'SLE9');
		@PKGS_TO_CHECK       = qw(bind bind-utils bind-devel);
		$FIXED_VERSION       = '9.3.4-4.10';
		...
		@PKGS_TO_CHECK       = qw(bind-utils bind-libs bind-libs-32bit);
		$FIXED_VERSION       = '9.3.4-1.26';
		SDP::Core::printDebug('ARGS', "$CHECKING, $ADVISORY, $TYPE, $#PKGS_TO_CHECK, $FIXED_VERSION");
		SDP::SUSE::securitySeverityPackageCheck($CHECKING, $ADVISORY, $TYPE, \@PKGS_TO_CHECK, $FIXED_VERSION);
	} else {
